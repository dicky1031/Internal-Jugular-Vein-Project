...
for wl_idx, wl in enumerate(wavelength):
    mua_set = np.zeros((20*len(bloodConc)*len(test_SO2)*len(muscle_SO2), 9))
    for bc_idx, bc in enumerate(bloodConc):
        for SO2_idx, using_SO2 in enumerate(test_SO2):
            for muscle_idx, muscle_SO2_used in enumerate(muscle_SO2):
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 ,0] = skin_mua[f'{wl}nm']
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,1] = fat_mua[f'{wl}nm']
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,2] = muscle_mua_spectrum[f'muscle_bloodConc_{bc}_bloodSO2_0.7'][wl_idx]
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,4] = CCA_mua[f'{wl}nm']
                
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,3] = mua_spectrum[f'ijv_bloodConc_{bc}_bloodSO2_{using_SO2}'][wl_idx]
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,5] = bc
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,6] = using_SO2
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,7] = muscle_mua_spectrum[f'muscle_bloodConc_{bc}_bloodSO2_{muscle_SO2_used}'][wl_idx]
                mua_set[muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20 : 20 + muscle_idx*20 + SO2_idx*len(muscle_SO2)*20 + bc_idx*len(test_SO2)*len(muscle_SO2)*20  ,8] = muscle_SO2_used
    np.save(os.path.join('OPs_used', f'{wl}nm_mua_set.npy'), mua_set)
    mua_set = pd.DataFrame(
        mua_set, columns=['skin', 'fat', 'muscle', 'ijv', 'cca', 'bloodConc', 'ijv_SO2', 'change_muscle_mua','muscle_SO2'])
