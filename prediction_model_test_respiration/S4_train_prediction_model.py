...
if __name__ == "__main__":
    ...
    # test_num = 200
    subject = 'ctchen'
    pretrain_model_folder = "prediction_model_formula3"
    result_folder = "prediction_model_formula3_train_on_mcx_again"
    train_folder = os.path.join("dataset", "ctchen", "ijv_depth_standard", "ijv_size_standard", "low_scatter_prediction_input", "all_absorption")
    test_folder = os.path.join("dataset", "ctchen", "test_dataset")
    assert os.path.isdir(train_folder), "train_folder path wrong!"
    assert os.path.isdir(test_folder), "test_folder path wrong!"
    #%%
    EPOCH = 40
    BATCH_SIZE = 512
    lr=0.00005
    ...
    model.load_state_dict(torch.load(os.path.join("model_save/ctchen/prediction_model_formula3/ep_8_loss_0.0001495952673392361.pth")))
