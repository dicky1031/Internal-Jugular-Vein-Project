...
def gen_surrogate_result(bloodConc:list, used_SO2:list, mus:dict, mua:dict, train_or_test:str, rangdom_gen:list, id:int):
    for blc in bloodConc:
        for s in used_SO2:
            os.makedirs(os.path.join("dataset", "surrogate_result", train_or_test, f'bloodConc_{blc}', f'SO2_{s}'), exist_ok=True)
    for blc in bloodConc:
...
if __name__ == "__main__":
    ...
    for id in range(test_num):
        ...
        gen_surrogate_result(bloodConc, test_SO2, mus, mua, "test", rangdom_gen, id)
