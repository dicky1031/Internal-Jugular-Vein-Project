...
for muscle_type in muscle_types:
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_train', 'low_absorption'), exist_ok=True)
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_train', 'medium_absorption'), exist_ok=True)
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_train', 'high_absorption'), exist_ok=True)
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_train', 'all_absorption'), exist_ok=True)
    
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_test', 'low_absorption'), exist_ok=True)
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_test', 'medium_absorption'), exist_ok=True)
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_test', 'high_absorption'), exist_ok=True)
    os.makedirs(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_test', 'all_absorption'), exist_ok=True)
    ...
    for wl_idx in range(len(wavelength)):
        dataset_large = pd.read_csv(os.path.join('dataset', 'kb', f'kb_dataset_large_{muscle_type}', 'low', f'{wavelength[wl_idx]}nm_mus_{wl_idx+1}.csv'))
        dataset_small = pd.read_csv(os.path.join('dataset', 'kb', f'kb_dataset_small_{muscle_type}', 'low', f'{wavelength[wl_idx]}nm_mus_{wl_idx+1}.csv'))
    ...
    np.save(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_test', 'all_absorption', 'prediction_input_test.npy'), all_prediction_input_test)
    
    prediction_input_train = pd.DataFrame(prediction_input_train)
    prediction_input_train.to_csv(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_train', 'all_absorption', 'prediction_input_train.csv'), index=False)
    all_prediction_input_train = prediction_input_train.to_numpy()
    np.save(os.path.join('dataset', 'kb', f'low_scatter_prediction_input_{muscle_type}_train', 'all_absorption', 'prediction_input_train.npy'), all_prediction_input_train)
